...
# enable postgis and run ckan datastore init (output from `ckan datastore set-permissions')
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" -d "$DATASTORE_DB" <<-EOSQL
  ...
  REVOKE CONNECT ON DATABASE "$CKAN_DB" FROM "$DATASTORE_READONLY_USER";
